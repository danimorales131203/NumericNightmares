...
public class Snake3 : MonoBehaviour // Define la clase Snake que hereda de MonoBehaviour
{
    ...
    private string[] eatMessages = { // Mensajes mostrados al comer
        "Fórmula de integral de una función logarítmica", "∫x/1 **=**∣x∣+*", "∫x/1 **=ln∣x∣+C", "∫  x/1 dx=ln∣x∣+C"
    };
    ...
    {
        SceneManager.LoadScene("PhoneCall3"); // Cambia a la escena especificada
    }
}