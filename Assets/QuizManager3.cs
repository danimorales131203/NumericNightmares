...
public class QuizManager3 : MonoBehaviour
{
    ...
    void GameOver()
    {
        ...
        {
            SceneManager.LoadScene("FinalTest"); // Carga la escena de todas correctas
        }
        else if (wrongAnswers == totalQuestions)
        {
            SceneManager.LoadScene("DeathScene3"); // Carga la escena de todas incorrectas
        }
        else
        {
            ...
            {
                SceneManager.LoadScene("FinalTest"); // Carga la escena de éxito
            }
            else
            {
                SceneManager.LoadScene("DeathScene3"); // Carga la escena de fracaso
            }
        }
    }
    ...
    void SetAnswers()
    {
        ...
        {
            options[i].GetComponent<Image>().color = options[i].GetComponent<AnswerScript3>().startColor; // Restaura el color de las opciones
            options[i].GetComponent<AnswerScript3>().isCorrect = false; // Establece que la respuesta no es correcta
            ...
            {
                options[i].GetComponent<AnswerScript3>().isCorrect = true; // Marca la respuesta correcta
            }
        }
    }
