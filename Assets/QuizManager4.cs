...
public class QuizManager4 : MonoBehaviour
{
    ...
    void GameOver()
    {
        ...
        {
            SceneManager.LoadScene("GoodEnding"); // Carga la escena de todas correctas
        }
        else if (wrongAnswers == totalQuestions)
        {
            SceneManager.LoadScene("BadEnding"); // Carga la escena de todas incorrectas
        }
        else
        {
            ...
            {
                SceneManager.LoadScene("GoodEnding"); // Carga la escena de éxito
            }
            else
            {
                SceneManager.LoadScene("BadEnding"); // Carga la escena de fracaso
            }
        }
    }
    ...
    void SetAnswers()
    {
        ...
        {
            options[i].GetComponent<Image>().color = options[i].GetComponent<AnswerScript4>().startColor; // Restaura el color de las opciones
            options[i].GetComponent<AnswerScript4>().isCorrect = false; // Establece que la respuesta no es correcta
            ...
            {
                options[i].GetComponent<AnswerScript4>().isCorrect = true; // Marca la respuesta correcta
            }
        }
    }
