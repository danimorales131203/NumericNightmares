...
public class QuizManager2 : MonoBehaviour
{
    ...
    void GameOver()
    {
        ...
        {
            SceneManager.LoadScene("Level3"); // Carga la escena de todas correctas
        }
        else if (wrongAnswers == totalQuestions)
        {
            SceneManager.LoadScene("DeathScene2"); // Carga la escena de todas incorrectas
        }
        else
        {
            ...
            {
                SceneManager.LoadScene("Level3"); // Carga la escena de éxito
            }
            else
            {
                SceneManager.LoadScene("DeathScene2"); // Carga la escena de fracaso
            }
        }
    }
    ...
    void SetAnswers()
    {
        ...
        {
            options[i].GetComponent<Image>().color = options[i].GetComponent<AnswerScript2>().startColor; // Restaura el color de las opciones
            options[i].GetComponent<AnswerScript2>().isCorrect = false; // Establece que la respuesta no es correcta
            ...
            {
                options[i].GetComponent<AnswerScript2>().isCorrect = true; // Marca la respuesta correcta
            }
        }
    }
